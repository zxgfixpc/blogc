<template>
  ...
  >
    ...
    <el-menu-item
      v-show="false"
      index="/home/article-detail"
    ></el-menu-item>

    <!-- 使用 v-for 渲染菜单项 -->
    <template
      class="homeMenu"
      v-for="(firstRouter, index) in navRouterObj.children"
      :key="index"
    >
      <!-- 一级菜单 -->
      <el-menu-item
        v-if="!firstRouter.children?.length && isShowMenu(firstRouter.meta.showType)"
        :index="firstRouter.meta.allPath"
      >
        <el-icon>
      ...
      <template v-if="firstRouter.children?.length && isShowMenu(firstRouter.meta.showType)">
        <el-sub-menu>
          ...
          >
            <!-- 二级页面路由 -->
            <el-menu-item
              v-if="!item.meta.isClick"
              :index="item.meta.allPath"
            >
              <el-icon>
